...
linuxarm32:
	sudo apt-get install gcc-multilib-arm-linux-gnueabihf && sudo ln -s /usr/include/asm-generic /usr/include/asm && GOOS=linux CGO_ENABLED=1 CC=arm-linux-gnueabihf-gcc GOARCH=arm go build -ldflags "${BUILD_VARS}" -o bin/glauth-arm32 ${BUILD_FILES} && cd bin && sha256sum glauth-arm32 > glauth-arm32.sha256
